...
class Fish:
    ...
    def update(self):
        # Move the fish: randomly, diagonally, bounces, and goes the opposite direction
        screen_info = pygame.display.Info()
        self.rect.move_ip(self.speed)
        # if fish touches left or right edge of screen
        if self.rect.left < 0 or self.rect.right > screen_info.current_w:
            self.speed[0] *= -1
            self.image = pygame.transform.flip(self.image, True, False)
            self.rect.move_ip(self.speed[0], 0)
        # if fish touches top or bottom edge of screen
